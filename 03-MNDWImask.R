...
mask.fun <- function(maskraster,index,choice,overwrite){
  ...
  if(!file.exists(paste("02-",index,"mask-",choice,".tif",sep=""))==TRUE){
    ...
    # Set land and water thresholds for boundaries, peaks or means of clusters
    if(choice == "Edge"){
      landthreshold <- max(df.land$data)
      waterthreshold <- min(df.water$data)
    }
    if(choice == "Peak"){
      landthreshold <- peak.land$dx
      waterthreshold <- peak.water$dx
    }
    
    if(choice == "Mean"){
      landthreshold <- mean(df.land$data)
      waterthreshold <- mean(df.water$data)
      #landthreshold <- (max(df.land$data) + peak.land$dx)/2
      #waterthreshold <- (peak.water$dx + min(df.water$data))/2
    }
