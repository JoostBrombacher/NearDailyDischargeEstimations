Qestlong.fun <- function(area,name,period,ts.period){
  ...
  # No training dataset
  start <- as.POSIXct("2017-10-01 00:00:00")
  notraining <- subset(data, date > start)
  ...
  RMSEnt <- formatC(signif(rmse(notraining$Q,notraining$est.rating),digits=3), digits=3,format="fg", flag="#")
  KGEnt <- formatC(round(KGE(notraining$Q,notraining$est.rating),digits=3), digits=3,format="fg", flag="#")
  RMSEw <- formatC(signif(rmse(winter$Q,winter$est.rating),digits=3), digits=3,format="fg", flag="#")
  KGEw <- formatC(round(KGE(winter$Q,winter$est.rating),digits=3), digits=3,format="fg", flag="#")
  RMSEnw <- formatC(signif(rmse(nowinter$Q,nowinter$est.rating),digits=3), digits=3,format="fg", flag="#")
  KGEnw <- formatC(round(KGE(nowinter$Q,nowinter$est.rating),digits=3), digits=3,format="fg", flag="#")
  ...
  p1 <- ggplot(data, aes(date)) +
    ...
    geom_line(aes(y=Q, colour = "  Observed"),size=0.75) + 
    coord_cartesian(xlim = c(min(data$date)-0.03*(max(data$date)-min(data$date)),max(data$date)+0.03*(max(data$date)-min(data$date))), ylim = c(150,850), expand = FALSE)+
  ...
  pres <- ggplot(data, aes(date,res)) +
    ...
    coord_cartesian(xlim = c(min(data$date)-0.03*(max(data$date)-min(data$date)),max(data$date)+0.03*(max(data$date)-min(data$date))), ylim = c(-300,250), expand = FALSE)+
