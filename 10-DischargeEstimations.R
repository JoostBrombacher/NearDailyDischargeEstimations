Qestval.fun <- function(area,name,period,overwrite){
  ...
  s3 <- ggplot(data, aes(Q,We),theme(plot.background = element_rect(fill = "grey90"))) +
    ...
    scale_color_manual(values=c("red"), 
                       labels=c(bquote(" "~ W[e] == .(signif(coefficients$a[1],2))~Q^~.(round(coefficients$b[1],2))~" "))) +
    annotate("text", -Inf, Inf, hjust =-0.13, vjust = 3.75,
             label = paste("paste(RMSE, \" = ",round(RMSE.scat,1),"\")",sep=""),
             parse = TRUE,size=5.5) +
    theme_stata() +
  ...
  s4 <- ggplot(data, aes(date)) +
    ...
    annotate("text", x = min(data$date)+0.8*(max(data$date)-min(data$date)),y = min(data$est.Q05,na.rm=TRUE)+0.9*(max(data$est.Q95,na.rm=TRUE)-min(data$est.Q05,na.rm=TRUE)),
             label = paste("paste(KGE, \" = ",round(KGE,3),"\")",sep=""),
             parse = TRUE,size=5.5) +
    annotate("text", x = min(data$date)+0.8*(max(data$date)-min(data$date)),y = min(data$est.Q05,na.rm=TRUE)+0.825*(max(data$est.Q95,na.rm=TRUE)-min(data$est.Q05,na.rm=TRUE)),
