...
val.fun <- function(d,masks,season,modelname,overwrite){
  ...
  if(overwrite==TRUE){
    ...
    class <- raster(paste(default,"/Data/Output/05-Classification/",modelname,"/",season,"/ValidationTraining/Pjorsa-20170820.grd",sep=""))
    ...
    name <- paste("06-ValidationResult_Clean.tif",sep="")
    ...
    accuracy.list <- c(accuracy(class))
    name.list <- c("Clean")
    for(i in 1:length(masks)){
      ...
      filename <- paste("06-ValidationResult_",name,".tif",sep="")
      writeRaster(class.masked,filename=filename,format="GTiff",overwrite=TRUE)
      
      accuracy.list <- c(accuracy.list,accuracy(class.masked))
      name.list <- c(name.list,name)
    }
  
    d <- data.frame(Type=name.list,Accuracy=accuracy.list)
